...
app.post("/", isRole(["kasir"]), async (req, res) => {
  try {
    ...
    // Periksa apakah user tersedia atau tidak
    ...
    // Check if each menu item exists
    for (let i = 0; i < listMenu.length; i++) {
      ...
      if (!getMenu) {
        return res.status(404).json({
          message: `Menu ID ${listMenu[i]["id_menu"]} tidak ditemukan`,
        });
      }
    }
...
app.put("/updatePembayaran/:id_transaksi", isRole(["admin","kasir"]), async (req, res) => {
  const param = { id_transaksi: req.params.id_transaksi };
  console.log(param,"india")
...
app.get("/filter/tgl_transaksi/:tgl_transaksi", isRole(["manajer"]), async (req, res) => {
  try {
    const { tgl_transaksi } = req.params;
    
    // Validate the input date
    if (!tgl_transaksi || isNaN(new Date(tgl_transaksi))) {
    ...
    const startDate = `${tgl_transaksi} 00:00:00`;
    const endDate = `${tgl_transaksi} 23:59:59`;

    // Fetch data from the transaksi table based on date range
    const result = await transaksi.findAll({
      ...
      // Order the data by id_transaksi in descending order
      order: [['id_transaksi', 'DESC']], // Newest transactions come first
    });
